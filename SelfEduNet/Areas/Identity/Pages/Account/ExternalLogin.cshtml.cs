			...
			if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (result.Succeeded)
                    {
                        ...
                        Task.Run(async () =>
                        {
                            await _emailSender.SendEmailAsync(Input.Email, "Підтвердіть ваш акаунт",
                                $"Підтвердіть реєстрацію перейшовни по посиланню: <a href='{HtmlEncoder.Default.Encode(callbackUrl)}'>тик</a>");
                        });
